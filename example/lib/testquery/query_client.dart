...
import 'package:persist_signals/signals/persisted_complex_list_signal.dart';
...
class QueryClient {
  ...
  /// Create or get existing query - uses global defaults if durations not specified
  ///
  ...
  useQuery<TData extends Object?, TQueryFnData extends Object?>(
    ...
    // Return existing query if already created (React Query behavior)
    if (_queries.containsKey(queryKey)) {
      return _queries[queryKey] as Query<TData, TQueryFnData>;
    }
  ...
  Future<T?> getCachedData<T>(
    ...
    try {
      ...
      // For primitive types, store directly without JSON encoding
  ...
  void addQueryListItem<TList extends List<TItem>, TItem>(
    ...
    final currentData = getQueryData<TList>(key);
    final list = currentData != null
        ? List<TItem>.from(currentData)
        : <TItem>[];
  ...
  Future<void> waitForHydration() async {
    final hydrationFutures = _queries.values
        .map((query) => query.waitForHydration())
        .toList();

    if (hydrationFutures.isNotEmpty) {
      await Future.wait(hydrationFutures);
    }
  }
  ...
  Future<void> waitForQueriesHydration(List<List<dynamic>> queryKeys) async {
    final hydrationFutures = queryKeys
